<script lang="ts">
    ...
    async function handleSave() {
        ...
        try {
            if (promptId) {
                ...
                const currentPrompt = $promptStore.prompts.find(
                    (p) => p.id === promptId,
                );
                if (currentPrompt) {
    ...
    function restoreVersion(v: any) {
        if (!confirm("이 버전으로 되돌리시겠습니까? 현재 내용은 사라집니다."))
            return;
        title = v.title;
        content = v.content;
        showHistory = false;
    ...
    async function handleAnalyze() {
        ...
        if (!apiKey) {
            showToast(
                "설정(톱니바퀴) > 보안 탭에서 API Key를 등록해주세요.",
                "error",
            );
            return;
        }
...
<div class="content-view">
    ...
    {#if aiSuggestion}
        <div
            class="ai-suggestion-box"
            style="background: var(--bg-tertiary); border: 1px solid var(--accent-blue); border-radius: 8px; padding: 16px; margin-bottom: 24px;"
        >
            <div
                style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;"
            >
                <h3
                    style="margin: 0; color: var(--accent-blue); display: flex; align-items: center; gap: 8px;"
                >
                ...
                <div style="display: flex; gap: 8px;">
                    <button
                        ...
                        style="background: var(--accent-blue); color: white; border: none; font-size: 0.85rem; padding: 6px 12px;"
                    >
                        적용하기
                    </button>
                    <button
                        class="button-like"
                        on:click={discardSuggestion}
            ...
            <div style="margin-bottom: 8px;">
                <span
                    style="font-weight: 600; font-size: 0.9rem; color: var(--text-secondary); width: 60px; display: inline-block;"
                    >제목:</span
                >
                <span style="color: var(--text-primary); font-weight: 500;"
                    >{aiSuggestion.title}</span
                >
            </div>
            <div>
                <span
                ...
                >
                <div
                    style="display: inline-flex; gap: 4px; flex-wrap: wrap; vertical-align: top;"
                >
                    {#each aiSuggestion.tags as tag}
                        <span
                            style="background: var(--bg-secondary); border: 1px solid var(--border-color); padding: 2px 8px; border-radius: 12px; font-size: 0.8rem;"
                            >#{tag}</span
                        >
                    {/each}
                </div>
    ...
    <div class="form-group" style="margin-bottom: 24px;">
        ...
        <input
            ...
            style="width: 100%; padding: 12px; background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 1rem;"
        />
    </div>

    <div class="form-group" style="margin-bottom: 24px;">
        <label style="display: block; margin-bottom: 8px; font-weight: 500;"
            >내용 {isTemplate ? "(플레이스홀더에 {{변수}} 사용)" : ""}</label
        >
        <textarea
            bind:value={content}
            style="width: 100%; height: 300px; padding: 12px; background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-family: var(--font-mono); font-size: 0.95rem; resize: vertical;"
        ></textarea>
        <div
    ...
    <div style="display:flex; gap: 24px; flex-wrap: wrap; margin-bottom: 24px;">
        <div class="form-group" style="flex:1;">
            ...
            <select
                bind:value={folderId}
                style="width: 100%; padding: 12px; background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary);"
            >
    ...
    <div
        class="is-template-toggle"
        style="background: var(--bg-tertiary); padding: 16px; border-radius: 8px; display: flex; align-items: center; gap: 12px;"
    >
        <input
            type="checkbox"
            id="isTemplate"
        ...
        />
        <label for="isTemplate" style="cursor: pointer; font-weight: 500;"
            >✨ 재사용 가능한 템플릿으로 만들기</label
        >
    </div>
</div>
