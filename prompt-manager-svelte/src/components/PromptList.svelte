<script lang="ts">
    ...
    async function toggleFavorite(e: Event, prompt: any) {
        e.stopPropagation();
        await promptStore.updatePrompt(prompt.id, { isFavorite: !prompt.isFavorite });
    }
...
<div>
    ...
    {:else}
        {#each filteredPrompts as prompt (prompt.id)}
            ...
            >
                <i class="prompt-list-icon fa-solid {prompt.isTemplate ? 'fa-puzzle-piece' : 'fa-file-lines'}" style="color: {prompt.isTemplate ? 'var(--accent-blue)' : 'var(--text-secondary)'}"></i>
                <i class="prompt-list-icon fa-solid fa-star fav-icon {prompt.isFavorite ? 'active' : ''}" on:click={(e) => toggleFavorite(e, prompt)}></i>
